...
void LexAnalyser::handleLeftRightAngleBra() {
    if (isTemplateClass(tokenRes[tokenRes.size() - 1].second.second) || searchKeyword(tokenRes[tokenRes.size() - 1].second.second) || isClassLexName(tokenRes[tokenRes.size() - 1].second.second)) { // 泛型处理逻辑:查找前一个标识符（类名）是否存在泛型
